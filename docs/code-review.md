...
- [ ] Fix `CacheStore.set()` evicting entries when updating an existing key at capacity
...
- [ ] Make TTL semantics explicit and consistent across layers
...
- [ ] Use monotonic time for durations and expiration math
...
- [ ] Validate and harden environment variable parsing (clear errors, tests)
...
- [ ] Remove dead/incorrect "error" handling branches in `server.py.Stats()`
...
- [ ] Replace broad `except:` clauses with `except Exception:` and avoid silent `pass`
...
- [ ] Fix `ConnectionInterceptor` semantics (it tracks RPCs, not connections)
...
- [ ] Fix client address reporting in interceptor logging
...
- [ ] Ensure `CacheStore.clear()` defines whether it resets stats counters
...
- [ ] Decide on `Delete` not-found semantics (response vs gRPC status)
...
- [ ] Use `asyncio.get_running_loop()` in `serve()` instead of `asyncio.get_event_loop()`
...
- [ ] Avoid blocking the event loop with `threading.Lock` inside async RPC handlers
...
- [ ] Reduce lock hold time during background TTL scans
...
- [ ] Ensure background cleaner thread lifecycle is fully owned by the process root
...
- [ ] Define a stable, typed status model for `CacheResponse.status`
...
- [ ] Validate key constraints (length/charset) and document them
...
- [ ] Enforce a maximum value size per entry
...
- [ ] Account for key and container overhead in memory accounting (or document limitations)
...
- [ ] Define behavior for values larger than total cache memory budget
...
- [ ] Preserve bytes end-to-end in the gRPC layer (or document current UTF-8 coercion)
...
- [ ] Align gRPC error responses with status codes
...
- [ ] Expose complete cache stats via gRPC (and keep `cache.proto` aligned)
...
- [ ] Consider implementing the standard gRPC health checking service
...
- [ ] Consider structured logging output (JSON) for production deployments
...
- [ ] Add correlation/request IDs across gRPC and HTTP logs
...
- [ ] Use `logger.exception(...)` (or `exc_info=True`) when logging unexpected errors
...
- [ ] Avoid returning raw exception messages to gRPC clients
...
- [ ] Consider exporting metrics (Prometheus) for hits/misses/evictions/memory
...
- [ ] Decide on the intended trust boundary (localhost-only sidecar vs network service) and document it
...
- [ ] Add TLS (and optionally mTLS) support for gRPC if used beyond localhost
...
- [ ] Restore or remove the missing `tests/test_grpc_service_mock.py` references
...
- [ ] Fix confusing test naming around `CacheEntry` mutability
...
- [ ] Fix `run_tests.py` options and messaging to match repository contents
...
- [ ] Add gRPC integration tests for Get/Set/Delete/Stats
...
- [ ] Add HTTP health endpoint tests
...
- [ ] Make TTL tests deterministic (avoid sleep-based flakiness)
...
- [ ] Fix `test_memory_based_eviction` to assert deterministic behavior
...
- [ ] Remove unused/duplicate imports in tests
...
- [ ] Make `test_thread_safety` result collection explicitly thread-safe
...
- [ ] Stop using `sys.path.insert(...)` in tests by packaging the project
...
- [ ] Ensure pytest markers are actually used or remove marker-based commands from docs
...
- [ ] Remove duplicated configuration parsing between `server.py` and `CacheStore`
...
- [ ] Make the health server bind address configurable (or consistent with `CACHE_HOST`)
...
- [ ] Consider `docker-compose.yml` defaults (log level, restart policy, env wiring)
...
- [ ] Add a `.dockerignore` to speed builds and avoid copying local artifacts
...
- [ ] Remove redundant Dockerfile copies and keep the image minimal
...
- [ ] Improve `Makefile` portability for protobuf generation
...
- [ ] Reduce Docker build-time dependency footprint in the builder stage
...
- [ ] Align local Python version expectations with container base image
...
- [ ] Add container/Kubernetes security hardening defaults
...
- [ ] Fix `docs/kubernetes-deployment.yaml` ServiceMonitor to scrape metrics (or remove it)
...
- [ ] Decide whether to keep both Dockerfile `HEALTHCHECK` and docker-compose `healthcheck`
...
- [ ] Keep `docs/TEST_README.md` aligned with the test runner and suite organization
...
- [ ] Remove emoji characters from `AGENTS.md` examples to match the "no emojis" rule
...
- [ ] Decide what to do with untracked `docs/todo.md`
